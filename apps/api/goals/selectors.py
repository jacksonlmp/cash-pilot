...
def get_goals_summary(user):
    ...
    if primary and primary.monthly_target > 0:
        progress_percent = (primary.current_amount / primary.monthly_target) * Decimal(
            "100"
        )
        missing_amount = max(
            primary.monthly_target - primary.current_amount, Decimal("0.00")
        )
    return {
        "title": primary.name if primary else "Meta: Reserva",
        "monthly_target": str(
            (primary.monthly_target if primary else Decimal("0.00")).quantize(
                Decimal("0.01")
            )
        ),
