...
def get_cards_summary(user):
    ...
    benefit_card = next(
        (card for card in cards if card.card_kind == Card.CardKind.BENEFIT), None
    )
    if benefit_card:
        usage_percent = Decimal("0.00")
        if benefit_card.total_limit > 0:
            usage_percent = (
                benefit_card.monthly_spend / benefit_card.total_limit
            ) * Decimal("100")
        benefit_summary = {
            ...
            "current_balance": str(
                benefit_card.current_balance.quantize(Decimal("0.01"))
            ),
